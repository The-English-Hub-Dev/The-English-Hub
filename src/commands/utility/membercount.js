...
class MembercountCommand extends Command {
    ...
    async chatInputRun(interaction) {
        ...
        const wantsFullScan = interaction.options.getBoolean('full');
        const wantsFastScan = interaction.options.getBoolean('fast');
        const forceScan = interaction.options.getBoolean('force');

        const canFullScan = interaction.member?.permissions?.has(
            PermissionFlagsBits.ManageGuild
        );
        const fullScanLimit = 100000;
        const isLargeGuild = guild.memberCount > fullScanLimit;
        ...
        if (wantsFastScan) {
            ...
            if (isLargeGuild && !forceScan) {
                doFullScan = false;
                scanNote =
                    'Full scan blocked for large servers. Re-run with --full and --force to override.';
            } else {
                doFullScan = true;
                scanNote = 'Full scan completed.';
            }
    ...
    registerApplicationCommands(registry) {
        const builder = new SlashCommandBuilder()
            ...
            .addBooleanOption((option) =>
                option
                    .setName('full')
                    .setDescription('Perform a full deep scan (slow)')
                    .setRequired(false)
            )
            .addBooleanOption((option) =>
                option
                    .setName('fast')
                    .setDescription('Force fast mode')
                    .setRequired(false)
            )
            .addBooleanOption((option) =>
                option
                    .setName('force')
                    .setDescription('Force full scan on large servers')
                    .setRequired(false)
            );
