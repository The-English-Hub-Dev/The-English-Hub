...
class RoleRemoveCommand extends Command {
    ...
    async chatInputRun(interaction) {
        ...
        if (!member.roles.cache.has(role.id)) return interaction.reply({ content: 'The member does not have that role.', ephemeral: true });
        await member.roles.remove(role, reason);
        return interaction.reply({ embeds: [new EmbedBuilder().setColor(Colors.Green).setDescription(`<:Hellos:1218430823229820968> Removed ${role} from ${member.user}.`)] });
    }
