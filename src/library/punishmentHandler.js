...
class PunishmentHandler {
	...
	async punish(user, duration, type, reason, ctx) {
		...
		if ([PunishmentType.MUTE, PunishmentType.UNMUTE, PunishmentType.KICK].includes(type) && !isMember) return container.utility.errReply(ctx.message, 'I cannot mute or kick a member not in this server.');
	...
	async memberChecks(member, ctx) {
		if (member.roles.highest.position >= ctx.message.member.roles.highest.position) {
			container.utility.errReply(ctx.message, 'You cannot punish someone equal or higher to you in hierarchy.');
			return false;
		}

		if (!member.manageable) {
			container.utility.errReply(ctx.message, 'I do not have permissions to punish this member.');
			return false;
		}
		return true;
	}
