async function runVcShortcut(interaction, container, channelId, unban = false) {
    ...
    if (interaction.member.roles.highest.position <= member.roles.highest.position) {
        return interaction.reply({
            content: 'You may not perform this action on members with equal or higher roles than you.',
            ephemeral: true,
        });
    }

    if (unban) {
        await channel.permissionOverwrites.delete(member, `VC unban by ${interaction.user.tag}`);
        await container.redis.hdel('vcban', `${channel.id}:${member.id}`);
        return interaction.reply(`${member} has been unbanned from ${channel}.`);
    }

    await channel.permissionOverwrites.edit(member, { Connect: false, SendMessages: false }, {
        reason: `VC ban by ${interaction.user.tag} (${interaction.user.id})`,
    });
    if (member.voice.channel === channel) await member.voice.disconnect('VC ban');
    await container.redis.hset('vcban', `${channel.id}:${member.id}`, Date.now());
